            ...
            <div className="dashboard-page" style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 16px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '24px' }}>
                    {filteredDoctors.length > 0 ? filteredDoctors.map((doctor, index) => (
                        <div key={index} style={{ background: 'var(--bg-secondary)', borderRadius: '16px', border: '1px solid var(--border-color)', padding: '24px', display: 'flex', flexDirection: 'column' }}>
                            ...
                            <h3 style={{ fontSize: '18px', fontWeight: '700', color: 'var(--text-primary)', margin: '0 0 4px 0' }}>{doctor.name}</h3>
                            <p style={{ fontSize: '14px', color: 'var(--text-secondary)', margin: '0 0 16px 0' }}>{doctor.specialty}</p>
                            
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '24px' }}>
                                <Star size={16} fill="var(--text-primary)" color="var(--text-primary)" />
                                <span style={{ fontSize: '14px', fontWeight: '600', color: 'var(--text-primary)' }}>{doctor.rating}</span>
                                <span style={{ fontSize: '14px', color: 'var(--text-secondary)' }}>({doctor.experience})</span>
                            </div>
                            
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                                <h2 style={{ fontSize: '24px', fontWeight: '700', color: 'var(--pill-success-text)', margin: 0 }}>{doctor.price}</h2>
